...
void create_insert_query(char *query) {
  ...
  input_int_field(query, PRICE);
  input_int_field(query, SHELF_LIFE);
  input_str_field(query, str_buffer, SORT); 
  input_str_field(query, str_buffer, MANUFACTURE_DATE);
  input_str_field(query, str_buffer, EXPIRY_DATE);
  input_int_field(query, CATEGORY_ID);
